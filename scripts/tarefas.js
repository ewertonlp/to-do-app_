...
btnFinalizaSessao.addEventListener('click', function () {
    Swal.fire({
        ...
        confirmButtonColor: '#4069e6',
        cancelButtonColor: '#d34545',
    ...
    fetch(`${urlAPI}users/getMe`, requestOptions)
        ...
        .catch(err => {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Falha ao validar as informações do usuario!'
              })
        })
...
function renderizarTarefas(listaTarefas) {
    ...
    listaTarefas.forEach(tarefa => {
        ...
        if (!tarefa.completed) {
            ...
            <p class="nome">${tarefa.description}</p>
            <div>
                <button><i id="${tarefa.id}" class="ri-arrow-go-back-line undo"></i></button>
                <button><i id="${tarefa.id}" class="ri-delete-bin-6-line delete"></i></button>
            </div>
            </div>
        </li>`
...
function excluirTarefa() {
    ...
    const icoExcluirTarefa = document.querySelector('.ri-delete-bin-6-line');
    ...
    icoExcluirTarefa.addEventListener('click', function (ev) {
        ...
        fetch(`${urlAPI}tasks/${id}`, settingsExcluir)
            .then(res => {
                console.log(res.status);
                // confirm(`Deseja deletar o arquivo?`);
                Swal.fire(
                    'Sucesso!',
...
function retornarTarefaPendente() {
    const icoUndoTarefa = document.querySelector('.ri-arrow-go-back-line');
